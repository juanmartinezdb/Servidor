...
//			}
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				try {
					...
					Optional<Fabricante> fabSelect = fabDAO.find(id);
					Optional<Integer> productos = fabDAO.getCountProductos(id);
					Optional<FabricanteDTO> fabDTO = fabSelect.map(fab -> new FabricanteDTO(fab,productos.orElse(0)));
